...
def is_equal_or_group_member(str1: str, str2: str) -> bool:
    ...
    group_members.extend(
        ...
        for part in re.split(r"と", member)
    )
...
def is_rts5_name_match(secondary_name: str, official_name: str) -> bool:
    ...
    return official_target is not None and is_equal_with_subst(official_target, official_name)
...
def main():
    ...
    soup = retrieve_soup_fetch_if_needed("https://web.archive.org/web/20110928120540/http://www.reitaisai.com/circlelists/", "raw_official.html")

    circles_info = []
    official_soups = []
    shortcut_urls = {
        urljoin(
            "https://web.archive.org/web/20110123213220/http://www.reitaisai.com/circlelists/",
            re.sub(r"/web/\d+", "/web/20110123213220", str(link["href"])),
        )
        for link in soup.find_all("a", href=True)
        if re.search(r"/positions/(?:[0-9]|[1-3][0-9]|4[0-3])$", urlparse(str(link["href"])).path)
    }
    for shortcut_url in sorted(shortcut_urls):
        shortcut_path = f"position_{Path(urlparse(shortcut_url).path).name}.html"
        official_soups.append(
            retrieve_soup_fetch_if_needed(shortcut_url, shortcut_path)
        )
    print(f"Found {len(official_soups)} official source pages")

    for official_soup in official_soups:
        for space_tag in official_soup.select(".space_01"):
            circle_tag = space_tag.find_next_sibling(class_="circle_01")
            pen_name_tag = space_tag.find_next_sibling(class_="pn_01")
            if circle_tag is None or pen_name_tag is None:
                continue
            position = space_tag.get_text(strip=True)
            circle_name = circle_tag.get_text(strip=True)
            pen_name = pen_name_tag.get_text(strip=True)
            description_parts: list[str] = []
            links = []
            hp_tag = circle_tag.find('a')
            if hp_tag and 'href' in hp_tag.attrs:
                links.append(remove_wayback_prefix(hp_tag['href']))
    ...
    secondary_links = [
        "https://thwiki.cc/%E5%8D%9A%E4%B8%BD%E7%A5%9E%E7%A4%BE%E4%BE%8B%E5%A4%A7%E7%A5%AD/%E7%AC%AC8%E5%B1%8A%E6%91%8A%E4%BD%8D/%EF%BB%BF%E3%81%82%EF%BD%9E%E3%81%95%E9%83%A8%E5%88%86",
        "https://thwiki.cc/%E5%8D%9A%E4%B8%BD%E7%A5%9E%E7%A4%BE%E4%BE%8B%E5%A4%A7%E7%A5%AD/%E7%AC%AC8%E5%B1%8A%E6%91%8A%E4%BD%8D/%E3%81%97%EF%BD%9E%E3%81%AB%E9%83%A8%E5%88%86",
        "https://thwiki.cc/%E5%8D%9A%E4%B8%BD%E7%A5%9E%E7%A4%BE%E4%BE%8B%E5%A4%A7%E7%A5%AD/%E7%AC%AC8%E5%B1%8A%E6%91%8A%E4%BD%8D/%E3%81%AC%EF%BD%9EF%E9%83%A8%E5%88%86",
        "https://thwiki.cc/%E5%8D%9A%E4%B8%BD%E7%A5%9E%E7%A4%BE%E4%BE%8B%E5%A4%A7%E7%A5%AD/%E7%AC%AC8%E5%B1%8A%E6%91%8A%E4%BD%8D/G%EF%BD%9EQ%E9%83%A8%E5%88%86",
    ]
    ...
    for secondary_index, secondary_url in enumerate(secondary_links):
        ...
        for row in secondary_soup.find_all("tr"):
            ...
            for index in official_name_index.get(normalize_for_match(circle_name), []):
                if is_equal_or_group_member(circle_name, circles_info[index]["circle_name"]):
    ...
    for info in circles_info:
        circles.append(
            Circle(
                ...
                description=info["description"] if is_to_add(info["description"]) else None,
                links=ensure_unicity(info["links"]) if is_to_add(info["links"]) else None,
                aliases=[info["circle_name"]],
                pen_names=[info["circle_penname"]] if is_to_add(info["circle_penname"]) else None,
            )
        )
