...
def main():
    ...
    for i, base_url in enumerate(
        ...
        table_div = official_soup.find("div", class_="post_content")
        if not table_div:
            continue
        ...
        for table in tables:
            for row in table.find_all("tr"):
                ...
                if "配置" in cells[0].text:
                    continue
                circle_name = sanitize_string(cells[0].get_text(strip=True))
                pen_name = sanitize_string(cells[1].get_text(strip=True))
