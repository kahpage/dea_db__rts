...
def retrieve_file_fetch_if_needed(
    url: str,
    file_name: str,
    fallback_url: str | None = None,
) -> Path:
    """Retrieve a file, using a fallback URL when the primary source rejects the request."""
    file_path = PATH_EVENT / file_name
    if not file_path.exists():
        print(f"File not found, fetching from {url} ...")
        headers = {"User-Agent": "Mozilla/5.0"}
        response = requests.get(url, headers=headers)
        if response.status_code != 200 and fallback_url is not None:
            print(f"Primary source returned {response.status_code}; fetching from fallback ...")
...
def main():
    ...
    # Official source
    # Parse the PDF document to extract circle information
    pdf_url = "https://web.archive.org/web/20160502130504/http://g-mirror.gptwm.com/reitaisai/reitaisai9_poslist.pdf"
    pdf_path = retrieve_file_fetch_if_needed(
        pdf_url,
        "raw_official.pdf",
        "http://g-mirror.gptwm.com/reitaisai/reitaisai9_poslist.pdf",
    )

    circles_info = []
    print(f"Reading circles from {pdf_path.name} ...")
    with pdfplumber.open(pdf_path) as pdf:
        for page in pdf.pages:
            tables = page.extract_tables({
                "vertical_strategy": "explicit",
                "explicit_vertical_lines": [51.4, 90, 330, 541.17],
            ...
            for row in tables[0][1:]:
                if len(row) != 3 or not row[0] or not row[1]:
                    continue
                position = sanitize_string(row[0])
                circle_name = sanitize_string(row[1])
                pen_name = sanitize_string(row[2]) if row[2] else ""
                if position == "配置":
                    continue
    ...
    print(f"Extracted {len(circles_info)} circles from official PDF")
    ...
    secondary_links = [
        ...
        "https://thwiki.cc/%E5%8D%9A%E4%B8%BD%E7%A5%9E%E7%A4%BE%E4%BE%8B%E5%A4%A7%E7%A5%AD/%E7%AC%AC9%E5%B1%8A%E6%91%8A%E4%BD%8D/N%EF%BD%9E%E3%81%8A%E9%83%A8%E5%88%86",
        "https://thwiki.cc/%E5%8D%9A%E4%B8%BD%E7%A5%9E%E7%A4%BE%E4%BE%8B%E5%A4%A7%E7%A5%AD/%E7%AC%AC9%E5%B1%8A%E6%91%8A%E4%BD%8D/%E3%81%8B%EF%BD%9E%E3%81%9D%E9%83%A8%E5%88%86",
        "https://thwiki.cc/%E5%8D%9A%E4%B8%BD%E7%A5%9E%E7%A4%BE%E4%BE%8B%E5%A4%A7%E7%A5%AD/%E7%AC%AC9%E5%B1%8A%E6%91%8A%E4%BD%8D/%E3%81%9F%EF%BD%9E%E3%81%AF%E9%83%A8%E5%88%86"
    ]
