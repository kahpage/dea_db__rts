...
def main():
    ...
    soup = retrieve_soup_fetch_if_needed("https://web.archive.org/web/20251012230741/http://www5b.biglobe.ne.jp/~cck/list/rei5_list_tu.htm", "raw_official.html", encoding="cp932")
    ...
    shortcut_urls = {
        ...
        if re.search(r"rei5_list_[a-z]+\.htm$", urlparse(link["href"]).path)
    }
    for shortcut_url in sorted(shortcut_urls):
        shortcut_path = Path(urlparse(shortcut_url).path).name
        if shortcut_path == "rei5_list_tu.htm":
            continue
        official_soups.append(
            retrieve_soup_fetch_if_needed(shortcut_url, shortcut_path, encoding="cp932")
        )
    print(f"Found {len(official_soups)} official source pages")

    for official_soup in official_soups:
        table = official_soup.find_all('table')[-1]
    ...
    for index, info in enumerate(circles_info):
        ...
        for alias, official_name in RTS5_SPECIAL_MATCHES.items():
            if normalize_for_match(official_name) == normalize_for_match(info["circle_name"]):
                official_name_index.setdefault(normalize_for_match(alias), []).append(index)

    # Secondary source
    # Parse the HTML content to extract circle information
    thwikicc = "https://thwiki.cc/%E5%8D%9A%E4%B8%BD%E7%A5%9E%E7%A4%BE%E4%BE%8B%E5%A4%A7%E7%A5%AD/%E7%AC%AC5%E5%B1%8A%E6%91%8A%E4%BD%8D"
    soup = retrieve_soup_fetch_if_needed(thwikicc,
            "raw_secondary.html",
            f"https://web.archive.org/web/2/{thwikicc}")
    ...
    for row in rows:
        ...
        candidate_indices = official_name_index.get(normalize_for_match(circle_name), [])
        for index in candidate_indices:
            info = circles_info[index]
            if is_rts5_name_match(circle_name, info["circle_name"]):
                circle_info = info
