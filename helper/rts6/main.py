...
def retrieve_soup_fetch_if_needed(
    ...
    if not html_path.exists():
        ...
        if response.status_code != 200 and fallback_url is not None:
            print(
                f"Primary source returned {response.status_code}; fetching from fallback (OUTDATED, consider providing the html file manually) !) ..."
            )
...
def is_equal_or_group_member(str1: str, str2: str) -> bool:
    ...
    group_members = re.split(r"[&+]", normalized_group)
...
def get_match_keys(name: str) -> set[str]:
...
def main():
    ...
    soup = retrieve_soup_fetch_if_needed(
        "https://web.archive.org/web/20260423021711/http://www.green.dti.ne.jp/maisan/gensoukyou/reitaisai6_list.html",
        "raw_official.html",
        encoding="cp932",
    )
    ...
    for index, info in enumerate(circles_info):
        for key in get_match_keys(info["circle_name"]):
            official_name_index.setdefault(key, []).append(index)

    # Secondary source
    # Parse the HTML content to extract circle information
    touhouwikicc = "https://thwiki.cc/%E5%8D%9A%E4%B8%BD%E7%A5%9E%E7%A4%BE%E4%BE%8B%E5%A4%A7%E7%A5%AD/%E7%AC%AC6%E5%B1%8A%E6%91%8A%E4%BD%8D/%E3%81%82%EF%BD%9E%E3%81%91%E9%83%A8%E5%88%86"
    soup = retrieve_soup_fetch_if_needed(
        touhouwikicc,
        "raw_secondary.html",
        f"https://web.archive.org/web/2/{touhouwikicc}",
    )
    ...
    unused_indices = [
        ...
        if circles_info[i]["circle_name"] and i not in circles_info_used_indices
    ]
