...
def retrieve_content_fetch_if_needed(
    url: str,
    file_name: str = "raw.html",
    fallback_url: str | None = None,
    encoding: str | None = None,
) -> str:
    """Retrieve raw page content, using a fallback URL when the primary source rejects the request."""
    ...
    with html_path.open("r", encoding=encoding or "utf-8", errors="replace") as f:
        return f.read()
...
def retrieve_soup_fetch_if_needed(
    ...
    """Retrieve a page as BeautifulSoup."""
    content = retrieve_content_fetch_if_needed(url, file_name, fallback_url, encoding)
...
def remove_span(txt: str) -> str:
    ...
    txt = re.sub(r"<span class='is-hidden'>.*?</span>", '', txt)
    return txt
...
def main():
    ...
    for i, base_url in enumerate(
        [
            "https://web.archive.org/web/20260512005320id_/https://reitaisai.com/rts21/circle-place-assign/",
        ]
    ):
        raw_content = retrieve_content_fetch_if_needed(
            base_url,
            f"raw_official_{i}.html",
        )
        re_data = re.compile(r'const data\s*=\s*(\[[\s\S]*?\])\s*</script>')
        ...
        if match:
            ...
            for entry in data:
                ...
                circle_position = remove_span(entry.get("space", "").strip())
                circle_web = remove_span(entry.get("web", "").strip())
                circle_pixiv = entry.get("pixiv", 0)
                circle_twitter = remove_span(str(entry.get("twitter", "")).strip())
                ...
                links: list[str] = []
                if circle_web:
                    circle_web = remove_wayback_prefix(circle_web)
                    links.append(circle_web)
                if circle_pixiv:
                    links.append(f"https://www.pixiv.net/users/{circle_pixiv}")
                if circle_twitter:
                    links.append(f"https://twitter.com/{circle_twitter}")
                if any((circle_name, circle_penname, circle_namekana, circle_pennamekana)):
                    circle = Circle(
                        ...
                        position=circle_position if is_to_add(circle_position) else None,
                        links=ensure_unicity(links) if is_to_add(links) else None
                    )
